...
parser.add_argument('--train_dir', type=str, default='/tr',
                    help='directory including mix.json, s1.json and s2.json')
parser.add_argument('--valid_dir', type=str, default='/cv',
                    help='directory including mix.json, s1.json and s2.json')
...
def pad_list(xs):
    ...
    if len(max_len) == 1:
        pad = np.zeros((n_batch, max_len[0]), np.float16)
    else:
        pad = np.zeros((n_batch, max_len[0], max_len[1]), np.float16)
